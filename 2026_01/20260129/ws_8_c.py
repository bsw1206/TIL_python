...
class ExtendedModel(Novel, Other):
    def __init__(self, data_type, title, content, created_at, updated_at, extended_type):
        super().__init__(data_type, title, content, created_at, updated_at, extended_type)
        self.extended_type = extended_type

    def display_info(self):
        self.extended_type = 'Extended Type'
        print('ExtendedModel 인스턴스의 정보 출력 및 저장 메서드 호출')
        print(f'PK: {self.PK}, TYPE: {extended_instance.TYPE}, Extended Type: {extended_instance.extended_type}')
    def save(self):
...
extended_instance = ExtendedModel('클래스', '다중 상속', '정말', '어렵네요', '더 열심히', '공부하겠습니다.')
